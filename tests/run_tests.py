...
def run_demo() -> int:
    ...
    for path in ["scripts/backup.sh", "scripts/restore.sh", "scripts/check_replication.sh", "scripts/seed_demo_data.sh", "scripts/backup_verify.sh"]:
...
def run_production() -> int:
    ...
    env = dict(os.environ)
    env.setdefault("COMPOSE_PROJECT_NAME", "repo17_prodtests")
